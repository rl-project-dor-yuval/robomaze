...
class Plot(object):
    def __init__(self, filename, map_PILobj=None):
        """
        Create a plot
        :param filename: filename0
        """
        self.filename = "../RRT_Star/visualizations/" + filename + ".html"
        self.data = []
    ...
    def draw(self, auto_open=True):
        """
        Render the plot to a file and if a map image was inserted ,
        show it
        """
        if isinstance(self.map, PngImagePlugin.PngImageFile):
            f = plotly.subplots.make_subplots(rows=1, cols=1)
            # Plot algorithms output
            for g_obj in self.data:
                f.add_trace(g_obj)

            # add maze picture besides
            f.add_layout_image(
                dict(
                    source=self.map,
                    xref="x",
            ...
            f.update_layout(title_text="Plot", template="plotly_white")
            py.offline.plot(f, filename=self.filename, auto_open=auto_open)
