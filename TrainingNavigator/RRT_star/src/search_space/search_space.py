...
from TrainingNavigator.RRT_star.src.utilities.geometry import es_points_along_line
from TrainingNavigator.RRT_star.src.utilities.obstacle_generation import obstacle_generator
...
class ImgSearchSpace(SearchSpace):

    def __init__(self, dimension_lengths, O, Im: np.ndarray):
        super().__init__(dimension_lengths, O)
        self.image = Im

    def obstacle_free(self, x):
        """
        Check if a location resides inside of an obstacle
        :param x: location to check (x[0],x[1])
        :return: True if not inside an obstacle, False otherwise
        """
        return self.image[int(np.floor(x[0])), int(np.floor(x[1]))] == 0
