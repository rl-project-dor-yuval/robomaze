...
if __name__ == '__main__':
    ...
    config_temp = yaml.load(open("TrainingNavigator/configs/BN_NoKillOnWall.yaml", "r"), yaml_loader)

    # add all config parameters to argparse:
    parser = argparse.ArgumentParser()
    for key in config_temp:
        parser.add_argument('--' + key, type=type(config_temp[key]),)
    # add rewards seperately:
    reward_dict = vars(config_temp["rewards"])
    ...
    for key in args.__dict__:
        ...
        elif key in config.keys() and key != "rewards":
